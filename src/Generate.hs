...
randomSD' outermost c leastTwoLevels ns alphabet (l,nm,mustCD) exclude = do
  ...
  subTypes1 <- vectorOf n ( if c > 0 then chooseNodeTypes else chooseNoSubNodeTypes) 
...
randomJointConnection layerElem innerElem globalStarts subs joint = do 
    ...
    let cds =filter (not.(`notCD` subs)) noHistory
    cd <- elements (filter (inCompoundState joint) cds)  
    let cdSub = filter (\ x -> take (length cd) x == cd) noHistory \\ [cd]
    jointOut <- vectorOf fromNum (elements (if fromNum == 1 then noParallelRegionHistoryElem \\[joint] else cdSub \\[joint]))
    ...
    if joint `elem` globalStarts
      ...
      let toNum = if fromNum == 1 then 2 else 1 
          ...
          noParallelRegionEndElem = filter (\x -> checkParallelRegion joint x subs) noEndState
          cdSubsNoEndState = filter (`isNotEnd` subs) cdSub
      jointIn <- vectorOf toNum (elements (if toNum == 1 then noParallelRegionEndElem\\[joint] else cdSubsNoEndState \\[joint] ))
                     `suchThat` (\x -> length (nub x) == length x && checkDistinctRegion cd x )
